// Edge function: injects XEKIE-specific OG tags before page is served
// Runs at the edge (before browser/crawler sees the page) so Facebook gets real meta tags
...
export default async function handler(request, context) {
  ...
  // No ID — just serve the page normally
  if (!xekieId) return context.next();

  // Only inject for crawlers/bots — real users get the page as-is (faster)
  ...
  try {
    // Fetch XEKIE data from Supabase
    ...
    // Replace generic OG tags with XEKIE-specific ones
    html = html
    ...
    // On any error, just serve the page normally
    return context.next();
  }
}
